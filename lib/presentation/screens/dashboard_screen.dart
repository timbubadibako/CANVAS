...
class _DashboardScreenState extends State<DashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    ...
    await Future.wait([
      Future.microtask(() => context.read<ProfileBloc>().add(LoadProfileRequested(userId))),
      _foodRepo.getTodayLogs(userId).then((logs) {
        if (mounted) setState(() { _todayLogs = logs; });
      }),
      _foodRepo.getRecentLogs(userId, limit: 3).then((logs) {
        if (mounted) setState(() { 
          _recentLogs = logs; 
                ...
                return SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _stagger(2, _buildRecentLogsHeader(isDark)),
                      ...
                      if (_isLoadingLogs)
                        ...
                        ..._recentLogs.asMap().entries.map((entry) {
                          ...
                          return Padding(
                            ...
                            child: _stagger(3 + entry.key, _buildLogItem(context, log, isDark)),
                          );
                        }),
    ...
    return Center(
      child: Column(
        children: [
          ...
          Icon(LucideIcons.list, color: isDark ? Colors.white10 : Colors.black12, size: 48),
  ...
  Widget _buildHeader(BuildContext context, bool isDark, ProfileState profileState) {
    ...
    if (profileState is ProfileLoaded) {
      ...
      final authState = context.read<AuthBloc>().state;
      if (authState is AuthGuest) name = "Guest";
    }
    ...
    return Row(
      ...
      children: [
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('DATE: $todayDate', style: Theme.of(context).textTheme.labelLarge?.copyWith(
              color: isDark ? AppColors.slateMuted : AppColors.lightMuted, fontSize: 10
            )),
            const SizedBox(height: 4),
            Text('Hello, $name', style: Theme.of(context).textTheme.headlineLarge?.copyWith(
  ...
  Widget _buildRecentLogsHeader(bool isDark) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text('Recent Logs', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: isDark ? Colors.white : AppColors.lightText)),
        GestureDetector(
          onTap: () => widget.onNavigateToTab(1), // Go to Diary
          child: const Text('VIEW ALL', style: TextStyle(color: AppColors.studioIndigo, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 1.2)),
        ),
      ],
    );
  }
  ...
  Widget _buildProgressCard(BuildContext context, bool isDark, ProfileState profileState) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('DAILY NUTRITION', style: TextStyle(color: AppColors.studioIndigo, fontWeight: FontWeight.w900, letterSpacing: 1.5, fontSize: 10)),
          ...
          const SizedBox(height: 32),
          _buildMacroProgress(isDark, 'Protein', '${_consumedProtein.toInt()}g', proTarget > 0 ? _consumedProtein/proTarget : 0, AppColors.studioIndigo),
          const SizedBox(height: 20),
          _buildMacroProgress(isDark, 'Carbs', '${_consumedCarbs.toInt()}g', carbTarget > 0 ? _consumedCarbs/carbTarget : 0, AppColors.vibrantEmerald),
        ],
      ),
    );
  }
  ...
  Widget _buildOrganicAvatar(bool isDark, String? imageUrl) {
    return Container(
      ...
      child: Container(
        ...
        child: imageUrl != null 
            ? (imageUrl.startsWith('http') ? Image.network(imageUrl, fit: BoxFit.cover) : Image.file(File(imageUrl), fit: BoxFit.cover))
  ...
  Widget _buildLogItem(BuildContext context, FoodLogEntry log, bool isDark) {
    return GestureDetector(
      onTap: () async {
        final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => MealDetailScreen(
          id: log.id,
          title: log.foodName,
            ...
            children: [
              ...
              Container(
                ...
                child: log.imageUrl == null
                    ? const Center(child: Icon(LucideIcons.utensils))
