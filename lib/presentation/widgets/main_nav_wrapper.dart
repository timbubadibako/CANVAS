...
class _MainNavWrapperState extends State<MainNavWrapper> with TickerProviderStateMixin {
  ...
  void _sendMessage() async {
    ...
    // 1. Ambil profile & data gizi hari ini secara real-time
    ...
    if (profileState is ProfileLoaded) {
      ...
      if (authState is AuthAuthenticated) {
        // Tarik data konsumsi hari ini agar bot "melek" gizi (REAL-TIME SYNC)
        final todayLogs = await FoodRepositoryImpl().getTodayLogs(authState.user.id);
        final consumedKcal = todayLogs.fold(0.0, (sum, item) => sum + item.caloriesKcal);
        final consumedProtein = todayLogs.fold(0.0, (sum, item) => sum + item.proteinG);
        final consumedCarbs = todayLogs.fold(0.0, (sum, item) => sum + item.carbsG);
        final consumedFat = todayLogs.fold(0.0, (sum, item) => sum + item.fatG);

        // 2. Bungkus pesan dengan konteks gizi lengkap
        finalPrompt = BotPromptDecorator.decorate(
          rawUserText, 
    ...
    try {
      ...
      print("[StudioBot] Gemini API Error: $e");
      if (mounted) {
        setState(() => _isBotThinking = false);
        StudioToast.show(context, 'STUDIO OFFLINE: $e', icon: LucideIcons.wifiOff);
      }
    }
  }
              ...
              child: Stack(
                ...
                children: [
                  GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        gradient: AppColors.emeraldGradient, shape: BoxShape.circle,
                        boxShadow: [BoxShadow(color: AppColors.vibrantEmerald.withValues(alpha: 0.3), blurRadius: 15, offset: const Offset(0, 8))],
                        border: Border.all(color: isDark ? AppColors.vibrantEmerald : Colors.white, width: 1),
                      ),
                      child: AnimatedSwitcher(
                        duration: const Duration(milliseconds: 300),
                        transitionBuilder: (child, animation) => ScaleTransition(scale: animation, child: child),
                        child: Icon(_isChatOpen ? LucideIcons.send : LucideIcons.bot, key: ValueKey<bool>(_isChatOpen), color: Colors.white, size: 28),
                      ),
                    ),
                  ),
                  if (_hasNotification && !_isChatOpen)
                    Positioned(
                      top: 2, right: 2,
                      child: Container(height: 14, width: 14, decoration: BoxDecoration(color: AppColors.deepRose, shape: BoxShape.circle, border: Border.all(color: isDark ? AppColors.deepSlate : Colors.white, width: 2.5))),
                    ),
                ],
              ),
            ),
          ...
          return Opacity(
            ...
            child: Transform.scale(
              ...
              child: Container(
                ...
                child: Text(_activeReminder!, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isDark ? Colors.white : AppColors.lightText)),
              ),
            ),
          );
        },
    ...
    return AnimatedPositioned(
      ...
      child: ScaleTransition(
        ...
        child: FadeTransition(
          ...
          child: Container(
            width: MediaQuery.of(context).size.width - 64, height: 592,
            decoration: BoxDecoration(
              color: isDark ? AppColors.deepSlate : Colors.white,
              borderRadius: BorderRadius.circular(40),
              border: Border.all(color: AppColors.vibrantEmerald.withValues(alpha: 0.3)),
            ...
            child: Column(
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      if (index == _messages.length && _isBotThinking) return _buildThinkingBubble(isDark);
                ...
                Padding(
                  ...
                  child: TextField(
                    controller: _chatController, maxLines: 5, minLines: 1,
                    style: TextStyle(fontSize: 13, color: isDark ? Colors.white : AppColors.lightText),
                    decoration: InputDecoration(
                      hintText: 'Ask the Studio...', filled: true,
        ...
        children: [
          Flexible(
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isBot ? (isDark ? AppColors.slateCard : Colors.black.withValues(alpha: 0.05)) : AppColors.vibrantEmerald,
                borderRadius: BorderRadius.only(topLeft: const Radius.circular(20), topRight: const Radius.circular(20), bottomLeft: Radius.circular(isBot ? 4 : 20), bottomRight: Radius.circular(isBot ? 20 : 4)),
              ),
              child: Text(text, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: isBot ? (isDark ? Colors.white : AppColors.lightText) : Colors.white)),
            ),
          ),
        ],
      ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            decoration: BoxDecoration(color: isDark ? AppColors.slateCard : Colors.black.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(20)),
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Column(
          ...
          children: [
            AnimatedScale(scale: isActive ? 1.2 : 1.0, duration: const Duration(milliseconds: 400), curve: Curves.easeOutBack, child: Icon(icon, size: 24, color: isActive ? AppColors.studioIndigo : (isDark ? AppColors.slateMuted : AppColors.lightMuted))),
            if (isActive) ...[const SizedBox(height: 6), Container(height: 4, width: 4, decoration: const BoxDecoration(color: AppColors.studioIndigo, shape: BoxShape.circle))],
          ],
        ),
      ),
    );
